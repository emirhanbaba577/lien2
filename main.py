...
class ApplicationModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        btn = discord.ui.Button(label="Kanalı Kapat", style=discord.ButtonStyle.danger)
...
class TicketView(discord.ui.View):
    ...
    async def create_simple_ticket(self, interaction, label):
        if interaction.user.id in active_tickets:
            return await interaction.response.send_message("⚠️ Mevcut bir biletin zaten açık!", ephemeral=True)

        active_tickets.add(interaction.user.id)
        user_id_fixed = interaction.user.id # Closure için sabitlendi
        overwrites = { 
            interaction.guild.default_role: discord.PermissionOverwrite(read_messages=False), 
            interaction.user: discord.PermissionOverwrite(read_messages=True, send_messages=True), 
            interaction.guild.me: discord.PermissionOverwrite(read_messages=True, send_messages=True) 
        ...
        async def close_callback(i):
            ...
            if os.path.exists(file_name): os.remove(file_name)
            active_tickets.discard(user_id_fixed)
            await i.channel.delete()
    ...
    @discord.ui.button(label="Takım Başvurusu", style=discord.ButtonStyle.success)
    ...
    @discord.ui.button(label="Partnerlik", style=discord.ButtonStyle.primary)
